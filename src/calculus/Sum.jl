...
domain_array_type(S::Sum{K, C, D, L}) where {K, C <: AbstractArray, D, L} = domain_array_type(S.A[1])
domain_array_type(S::Sum{K, C, D, L}) where {K, C <: Tuple, D, L} = domain_array_type.(Ref(S.A[1]))
codomain_array_type(S::Sum{K, C, D, L}) where {K, C <: AbstractArray, D, L} = codomain_array_type(S.A[1])
codomain_array_type(S::Sum{K, C, D, L}) where {K, C <: Tuple, D, L} = codomain_array_type.(Ref(S.A[1]))
