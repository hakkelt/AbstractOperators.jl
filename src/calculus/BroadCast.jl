...
struct OperatorBroadCast{T, N, M, Threaded, Compact, Imask, L, C, D, K} <: AbstractBroadCast{T, N, M, Threaded}
    ...
    function OperatorBroadCast(
        ...
        threaded = threaded && _should_thread(domain_storage_type(A)) && Threads.nthreads() > 1
        N = ndims(A, 1)
        T = codomain_type(A)
        dim_in = size(A, 1)
...
has_mutable_buffers(::Type{<:OperatorBroadCast}) = true

function _copy_operator_impl(op::OperatorBroadCast; storage_type = nothing, threaded = nothing)
    return deepcopy(op)
end
