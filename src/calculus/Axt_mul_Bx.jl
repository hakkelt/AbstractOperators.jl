...
"""
struct Axt_mul_Bx{
        N, L1 <: AbstractOperator, L2 <: AbstractOperator, C <: AbstractArray, D <: AbstractArray,
    } <: NonLinearOperator
    A::L1
    B::L2
    bufA::C
    ...
    function Axt_mul_Bx(A::L1, B::L2, bufA::C, bufB::C, bufC::C, bufD::D) where {L1 <: AbstractOperator, L2 <: AbstractOperator, C <: AbstractArray, D <: AbstractArray}
        ...
        N = ndims(A, 1)
        return new{N, L1, L2, C, D}(A, B, bufA, bufB, bufC, bufD)
    end
end

struct Axt_mul_BxJac{
        N, L1 <: AbstractOperator, L2 <: AbstractOperator, C <: AbstractArray, D <: AbstractArray,
    } <: LinearOperator
    A::L1
    B::L2
...
function Axt_mul_Bx(A::AbstractOperator, B::AbstractOperator)
    ...
    return Axt_mul_Bx(A, B, bufA, bufB, bufC, bufD)
end

# Jacobian
function Jacobian(P::Axt_mul_Bx{N, L1, L2, C, D}, x::AbstractArray) where {N, L1, L2, C, D}
    JA, JB = Jacobian(P.A, x), Jacobian(P.B, x)
    return Axt_mul_BxJac{N, typeof(JA), typeof(JB), C, D}(
        JA, JB, P.bufA, P.bufB, P.bufC, P.bufD
    )
end
...
function size(P::Union{Axt_mul_Bx{2}, Axt_mul_BxJac{2}})
    return ((size(P.A, 1)[2], size(P.B, 1)[2]), size(P.A, 2))
end
...
# utils
function permute(
        P::Axt_mul_Bx{N, L1, L2, C, D}, p::AbstractVector{Int}
    ) where {N, L1, L2, C, D <: ArrayPartition}
    return Axt_mul_Bx(
        permute(P.A, p),
        permute(P.B, p),
        P.bufA,
...
function remove_displacement(P::Axt_mul_Bx)
    return Axt_mul_Bx(
