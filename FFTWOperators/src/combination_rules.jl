...
function combine(T1::DFT, T2::ShiftOp)
    return SignAlternation(codomain_type(T1), size(T1, 1), T2.dirs; array_type = codomain_storage_type(T1)) * T1
end
function combine(T1::ShiftOp, T2::DFT)
    return T2 * SignAlternation(domain_type(T2), size(T2, 2), T1.dirs; array_type = domain_storage_type(T2))
end
...
function combine(T1::FFTShift, T2::FFTShift)
    ...
    return FFTShift(domain_type(T1), size(T1, 2), new_dirs; array_type = domain_storage_type(T1))
end
function combine(T1::IFFTShift, T2::IFFTShift)
    new_dirs = Tuple(d for d in 1:ndims(T1, 2) if (d in T1.dirs) || (d in T2.dirs))
    return IFFTShift(domain_type(T1), size(T1, 2), new_dirs; array_type = domain_storage_type(T1))
end
function combine(T1::FFTShift, T2::IFFTShift)
    if does_fully_cover(T1, T2)
        new_dirs = Tuple(d for d in T1.dirs if !(d in T2.dirs))
        if isempty(new_dirs)
            return Eye(domain_type(T1), size(T1, 2); array_type = domain_storage_type(T1))
        else
            return FFTShift(domain_type(T1), size(T1, 2), new_dirs; array_type = domain_storage_type(T1))
        end
    else
...
function combine(T1::SignAlternation, T2::SignAlternation)
    ...
    if isempty(new_dirs)
        ...
        return SignAlternation(domain_type(T1), size(T1, 2), new_dirs; array_type = domain_storage_type(T1))
    end
end
