@testitem "Sum: basic mul" tags = [:calculus, :Sum] setup = [TestUtils] begin
    ...
    m, n = 5, 7
    A1 = randn(m, n); A2 = randn(m, n); A3 = randn(m, n)
...
@testitem "Sum: displacement" tags = [:calculus, :Sum] setup = [TestUtils] begin
    ...
    m, n = 5, 7
    A1 = randn(m, n); A2 = randn(m, n); A3 = randn(m, n)
    d1 = randn(m); d2 = pi; d3 = randn(m)
    opA1 = AffineAdd(MatrixOp(A1), d1); opA2 = AffineAdd(MatrixOp(A2), d2); opA3 = AffineAdd(MatrixOp(A3), d3)
    opS = Sum(opA1, opA2, opA3)
    x1 = randn(n)
...
@testitem "Sum: properties" tags = [:calculus, :Sum] setup = [TestUtils] begin
    ...
    m, n = 5, 7
    Aeq = MatrixOp(randn(m, n)); Beq = MatrixOp(randn(m, n))
    S1 = Sum(Aeq, Beq); S2 = Sum(Aeq, Beq); S3 = Sum(Beq, Aeq)
    @test S1 == S2 && S1 != S3
    @test Sum(Aeq) === Aeq
